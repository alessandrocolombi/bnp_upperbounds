...
if(save_img)
  pdf("img/Mod3_Inc4people_3IBP_paramfit.pdf", width = width, height = height)
...
var_nb = 100
start_params <- c(a = 1, b = 1, mu_nb = 100)
fit <- optim(par = start_params, fn = llik_MixBin,
             method = "L-BFGS-B",
             n = n, Kn = Kn, data_obs = N_j, var_nb = var_nb,
             lower = c(1e-10, 1e-10, 1e-10), upper = c(Inf, Inf, var_nb-1e-1))
a_mle = fit$par[1]
b_mle = fit$par[2] 
munb_mle = fit$par[3]
munb_mle
...
if(save_img)
  pdf("img/Mod3_Inc4people_MixBin_paramfit.pdf", width = width, height = height)
par(mfrow = c(1,1), mgp=c(2.5,0.5,0), mar = c(2,2.5,1,0), bty = "l", cex = 2)
plot(0,0,main = " ", ylab = "", yaxt = "n", 
     xlim = c(0,xmax), ylim = c(0,ymax), type = "n")
axis( side = 2, at = ypos, labels = ylabs, las = 1) 
...
## Summary ------------------------------------------------------------
c(ubFreqBdd, ubFreqUbd, ub3IBP, ubMixBin)
